...
class GoalChase:
    def __init__(self, *, spine, gate, registry, actor, verify_founder, clock,
                 router=None, sandbox=False, grant_for=None):
        ...
        # Only an independently supplied sandbox host may provision a grant.
        # No goal text or capability descriptor is itself a Kernel grant.
    ...
    def _perform(self, gid, action):
        ...
        standing_grant = (self.grant_for(proposal, goal["input"], self.decisions.get(rid))
                          if self.grant_for is not None else None)
        record = self.gate.run(proposal, executor=execute, standing_grant=standing_grant,
    ...
    def tick(self, trigger_id):
        ...
        for gid in active:
            ...
            if goal["conflicted"] or any(self.goals.get(d, {}).get("status") != "ACHIEVED"
                ...
                continue
            # Re-entry is safe only because _perform checks a retained start
            # before dispatch and _reconcile refuses an uncertain Gate result.
            # DurableWorkflow otherwise treats even a pre-dispatch approval wait
            # as a possibly completed, non-retryable step on current main.
            steps = [WorkflowStep(a["action_id"], lambda state, a=a: self._perform(gid, a),
                                  max_retries=0, retry_safe=True)
