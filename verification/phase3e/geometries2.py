"""Phase 3E geometry catalogue, CORRECTED.

`geometries.py` is preserved unchanged so run 1 stays exactly reproducible.

THE DEFECT IN run 1's CATALOGUE
-------------------------------

Several catalogues contained exactly ONE producer of the contract's output
type. The pre-registered damage policy selects a load-bearing carrier from the
execution trace, so it selected that unique producer and the function became
unrecoverable by construction. Those episodes measured the catalogue, not the
substrate.

Every geometry here provides at least two distinct ways to produce the output
type, so "restore the function" is a question the fixture can actually pose.
That is a property of a valid test, not a hint to the substrate: the cells are
still told only capability types and neighbourhoods, never which route to use.

`orchestration_correctly_superior` keeps the thinnest viable redundancy on
purpose - it exists so a planner handed the whole graph can beat local
recruitment, and that comparison is only meaningful if the episode is scorable.
"""
...
def recursively_missing_branch(rng):
    ...
    caps = (_fam_chain("nu") + _fam_chain("xi")
            + [cap("deep.a", ("NORM",), "MID", WRAP, 1.0, "d.deep", "deepen"),
               ...
               cap("rec.alt", ("CHK", "CHK"), "VERDICT", AGREE2, 1.2, "d.alt", "reconcile")])
...
def nested_quorum_delayed_evidence(rng):
    caps = (_fam_chain("nu") + _fam_chain("xi") + _fam_chain("om")
            + [cap("q", ("CHK", "CHK", "CHK"), "VERDICT", AGREE3, 1.0, "d.q", "quorum"),
               cap("q.alt", ("CHK", "CHK", "CHK"), "VERDICT", AGREE3, 1.2,
                   "d.qalt", "quorum")])
...
def orchestration_correctly_superior(rng):
    ...
    caps = [cap("s1", ("RAW",), "T1", NORM, 1.0, "d.s", "s1"),
            ...
            cap("s5b", ("CHK", "CHK"), "VERDICT", AGREE2, 1.4, "d.u", "s5")]
