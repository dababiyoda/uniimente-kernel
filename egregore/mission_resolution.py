...
class ResolutionCandidate:
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "ResolutionCandidate":
        ...
        return cls(
            ...
            expected_quality=float(value["expected_quality"]),
            estimated_cost_usd=float(value["estimated_cost_usd"]),
            coordination_units=float(value["coordination_units"]),
            founder_attention_minutes=float(value["founder_attention_minutes"]),
            reversible=bool(value["reversible"]),
            authority_delta=int(value.get("authority_delta", 0)),
            external_effects=int(value.get("external_effects", 0)),
    ...
    def route(
        ...
        *,
        ...
        for candidate in pool:
            ...
            if refusal is None:
                score, components = _score(candidate, forced_refusal=forced_refusal)
                eligible = True
