#!/usr/bin/env python3
"""Prove the Phase 3D pre-registration from git ancestry, not from self-report.
...
Exit non-zero on any failure. Skips cleanly when the Phase 3D files are absent
...
MANIFEST = "verification/phase3d/EVALUATION_MANIFEST.json"
IMPLEMENTATION = ["substrate/v2.py"]
RESULTS = ["verification/phase3d/PHASE3D_RESULTS.json"]
...
def main() -> int:
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        return 1
    print("\nOK: Phase 3D thresholds were fixed before the substrate that is "
