...
# answer, criticize, pause, resume or stop. Missions, capabilities, key rotation and device
# enrollment always need the founder key itself.
DEVICE_KINDS = ("DECISION", "CRITIQUE", "LIFECYCLE", "BODY_PAUSE", "BODY_RESUME", "BODY_STOP")
DEVICE_MAX_LIFETIME = timedelta(days=90)
READ_SKEW = timedelta(seconds=60)
...
def validate_device_grant(body: dict, *, now: datetime) -> dict:
    """The founder-signed body of DEVICE_ENROLL: which key, which kinds, until when."""
    if set(body) != {"public_key", "label", "kinds", "expires_at"}:
        raise FounderAuthError("device grant needs exactly public_key, label, kinds, expires_at")
    kid = key_id(body["public_key"])
    label = body["label"]
    if not isinstance(label, str) or not 1 <= len(label) <= 64:
        raise FounderAuthError("device label must be 1..64 characters")
    kinds = body["kinds"]
    if not isinstance(kinds, list) or not kinds or set(kinds) - set(DEVICE_KINDS) or len(set(kinds)) != len(kinds):
        raise FounderAuthError(f"device kinds must be a non-empty subset of {DEVICE_KINDS}")
    expires = instant(body["expires_at"])
    if not now < expires <= now + DEVICE_MAX_LIFETIME:
        raise FounderAuthError("device delegation must expire within 90 days")
    return {"device_key_id": kid, "public_key": body["public_key"], "label": label,
            "kinds": sorted(kinds), "expires_at": stamp(expires)}


def _read_bytes(body_id: str, method: str, path: str, at: str) -> bytes:
    return f"uniimente-greg-read-v1\n{body_id}\n{method}\n{path}\n{at}".encode("utf-8")


def sign_read(key: Ed25519PrivateKey, *, body_id: str, method: str, path: str,
              now: datetime | None = None) -> dict:
    """Headers for one authenticated read of the remote channel (short-lived, body-bound)."""
    at = stamp(now or datetime.now(timezone.utc))
    return {"X-Greg-Key": key_id(public_bytes(key.public_key()).hex()), "X-Greg-Time": at,
            "X-Greg-Signature": key.sign(_read_bytes(body_id, method, path, at)).hex()}


class FounderVerifier:
    """Verifies signed founder commands against the body's enrolled keys.

    ``enrolled`` maps key_id -> public key hex for founder keys that are currently valid.
    ``devices`` maps key_id -> founder-signed device delegation (see validate_device_grant).
    ``seen_nonce`` is a callable answering whether a nonce was already accepted;
    the body backs it with retained ledger history.
    """

    def __init__(self, *, body_id: str, enrolled: dict[str, str], seen_nonce, devices: dict | None = None):
        self.body_id, self.enrolled, self.seen_nonce = body_id, dict(enrolled), seen_nonce
        self.devices = dict(devices or {})

    def _public(self, kid: str, *, kind: str | None, now: datetime) -> tuple[str, str]:
        """(public key hex, principal) for a signer allowed to sign ``kind`` now."""
        if kid in self.enrolled:
            return self.enrolled[kid], "founder"
        device = self.devices.get(kid)
        if device is None:
            raise FounderAuthError("founder key not enrolled or revoked")
        if now >= instant(device["expires_at"]):
            raise FounderAuthError("device delegation expired")
        if kind is not None and kind not in device["kinds"]:
            raise FounderAuthError(f"device key may not sign {kind}; founder key required")
        return device["public_key"], "device:" + device["label"]

    def principal(self, envelope: dict, *, now: datetime | None = None) -> str:
        return self._public(envelope["founder_key_id"], kind=envelope["kind"],
                            now=now or datetime.now(timezone.utc))[1]

    def verify_read(self, headers, *, method: str, path: str, now: datetime | None = None) -> str:
        """Authenticate one read request; returns the principal. Reads never change state."""
        now = now or datetime.now(timezone.utc)
        kid, at, signature = (headers.get("X-Greg-Key"), headers.get("X-Greg-Time"),
                              headers.get("X-Greg-Signature"))
        if not (kid and at and signature):
            raise FounderAuthError("signed read required")
        public_hex, principal = self._public(kid, kind=None, now=now)
        if abs(instant(at) - now) > READ_SKEW:
            raise FounderAuthError("read signature too old or from the future")
        try:
            Ed25519PublicKey.from_public_bytes(bytes.fromhex(public_hex)).verify(
                bytes.fromhex(signature), _read_bytes(self.body_id, method, path, at))
        except (InvalidSignature, ValueError) as exc:
            raise FounderAuthError("read signature does not verify") from exc
        return principal
    ...
    def verify(self, envelope: dict, *, now: datetime | None = None,
        ...
        public_hex, _ = self._public(envelope["founder_key_id"], kind=envelope["kind"], now=now)
