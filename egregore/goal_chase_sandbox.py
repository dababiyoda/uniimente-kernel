...
def open_sandbox(path, *, founder, clock=None, router=None, genomes=None):
    """Use the canonical ledger's single-writer lock for the whole session.
    ...
    try:
        ...
        def sandbox_grant(proposal, signed_goal, signed_decision):
            """Provision a test-only pre-dispatch grant from exact synthetic input.

            Production callers do not get this callback. The public fixture key
            is never real founder authentication or live delegated authority.
            """
            scope = proposal.payload.get("scope", {})
            authorized_action = scope.get("action", {})
            if (proposal.payload.get("simulation") is not True
                    or not proposal.target.startswith("sandbox:")
                    or proposal.requested_capability not in ("research.compare", "prototype.simulate")
                    or not founder.verify(signed_goal)
                    or signed_goal.get("reality_status") != "SIMULATED"
                    or signed_goal.get("founder_identity") != "sandbox:alfonso"
                    or signed_goal["body"]["goal_id"] != proposal.objective
                    or scope.get("goal_id") != proposal.objective
                    or authorized_action not in signed_goal["body"]["actions"]
                    or proposal.requested_capability != authorized_action.get("capability")
                    or proposal.target != authorized_action.get("target")
                    or proposal.action_class != authorized_action.get("action_class")
                    or proposal.consequence_class != authorized_action.get("consequence_class")
                    or proposal.estimated_cost_usd != authorized_action.get("cost_cents", -1) / 100
                    or proposal.expected_outcome != authorized_action.get("expected_outcome")
                    or proposal.estimated_cost_usd > signed_goal["body"]["budget_boundary"]["ceiling_cents"] / 100):
                return None
            if proposal.requested_capability == "prototype.simulate":
                if (signed_decision is None or not founder.verify(signed_decision)
                        or signed_decision["body"].get("answer") != "APPROVE"
                        or signed_decision["body"].get("scope") != scope):
                    return None
            return gate.grants.issue_single_action(proposal=proposal,
        ...
        chase = GoalChase(spine=EventSpine(ledger), gate=gate, registry=genomes or registry(),
                          ...
                          router=router, sandbox=True, grant_for=sandbox_grant)
        yield chase
    finally:
        ledger.close()
