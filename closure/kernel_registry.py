...
def build_registry() -> ClosureRegistry:
    ...
    def _foundry_stack():
        ...
        from provenance.ledger import EvidenceLedger
        from provenance.commit_witness import WitnessSigner
        from foundry.company import (CompanyFoundry, MediaCompanyCharter,
        ...
        territory.add(ContentNode(node_id="entry", question="q1", artifact="essay",
                                  capability_taught="c1", evidence_level=0.9,
                                  evidence_refs=["sha256:" + "a" * 64],
                                  next_doors=["mid"]), entry=True)
        territory.add(ContentNode(node_id="mid", question="q2", artifact="tool",
                                  capability_taught="c2", evidence_level=0.85,
                                  evidence_refs=["sha256:" + "b" * 64], next_doors=["exit"]))
        territory.add(ContentNode(node_id="exit", question="q3", artifact="course",
                                  capability_taught="c3", evidence_level=0.8,
        ...
        charter = MediaCompanyCharter(
            ...
            visual_canon={"palette": "x"}, editorial_rules=list(REQUIRED_EDITORIAL_RULES),
            narrative_world="w", owned_hub="hub", subscriber_list="subs",
            products=["p"], community="commons")
        foundry = CompanyFoundry(ledger)
        h = foundry.submit_charter(charter, territory)
        return foundry, h, gate, ledger, actor

    LIVE = lambda p: {"observed_outcome": "artifact live on declared account",
                      "result_class": "positive"}

    def foundry_technical():
        foundry, h, gate, ledger, actor = _foundry_stack()
        ...
        return rec.state == "recorded" and rec.receipt_hash is not None, \
            "chartered company publishes a territory node through the full gate pipeline"
    ...
    def foundry_authority():
        ...
        try:
            ...
            pass
        foundry.ratifier.decide(h, ratified=True, reason="ok")
    ...
    def foundry_evidence():
        foundry, h, gate, ledger, actor = _foundry_stack()
        t = foundry.company(h).territory
        t.correct("mid", correction="superseded", new_evidence_level=0.9)
        t.retire("mid", reason="no longer defensible")
        ...
        return ok and not t.publishable("mid")[0], \
            "corrections and retirements are public ledger events; retired nodes cannot publish"

    def foundry_economic():
        from foundry.distribution import DistributionLoop
        loop = DistributionLoop("closure-media")
        w = loop.open_window("w1")
        w.record_impressions(1000)
        w.record_owned_relationship(10)
        w.record_returning_visitor(20)
        w.record_useful_action("used_tool", 8)
        result = loop.evaluate(w)
        return result.overall == "CLOSED" and w.informed_return() > 0, \
            "rented attention converts into owned relationships + capability: a durable asset"

    def foundry_regenerative():
        from foundry.distribution import DistributionLoop
        loop = DistributionLoop("closure-media")
        for n in (1000, 2000):
            w = loop.open_window(f"w{n}")
            w.record_impressions(n)
        vanity = loop.evaluate(loop.windows[-1])
        return vanity.overall == "FALSELY_CLOSED" and loop.kill_condition_met(), \
            "impressions without behavior change is named false closure and trips the kill condition"

    reg.register(ModuleClosures("foundry", {
    ...
    def _business_stack():
        ...
        compiler = BusinessGenomeCompiler(genome_registry=registry,
                                          ratifier=Ratifier(ledger), ledger=ledger)
        ...
        def genome(**kw):
            defaults = dict(
                ...
                marginal_cost_usd=120.0, demand_evidence_refs=["sha256:" + "e" * 64],
                ...
                falsification_test="10 offers in 90 days; zero acceptances kills")
            defaults.update(kw)
    ...
    def business_technical():
        ...
        loop.verify_outcome(case.case_id, verified_by="external_receipt",
                            detail="buyer's auditor accepted")
        loop.resolve(case.case_id, retained=True, reason="subscribed")
        return loop.evaluate().overall == "CLOSED", \
            "problem -> buyer -> offer -> payment -> delivery -> verified outcome -> retention"
    ...
    def business_authority():
        ...
        try:
            ...
            return True, "weak-evidence payment refused at the gate; no delivery before payment"
    ...
    def business_evidence():
        ...
        try:
            ...
            pass
        refused = [r.payload for r in ledger.by_type("event")
                   if r.payload.get("type") == "business.genome_refused"]
        return bool(refused), "genome refusals are preserved as negative evidence on the ledger"
    ...
    def business_economic():
        ...
        except GenomeCompileError as e:
            return "donation with paperwork" in str(e), \
                "a price that does not cover marginal cost refuses to compile"
    ...
    def business_regenerative():
        ...
        try:
            ...
            return False, "dead business took a new case"
        except CommercialLoopError:
            kills = [r.payload for r in ledger.by_type("event")
                     if r.payload.get("type") == "business.terminated"]
            return bool(kills) and "appreciating asset" in kills[0]["learning"], \
                "the precommitted kill executes; the failure is preserved as learning"

    reg.register(ModuleClosures("business", {
    ...
    def treasury_technical():
        t, ledger = _treasury()
        tiers = t.waterfall
        alloc = t.allocate(600.0, {tiers[0]: 300.0, tiers[1]: 200.0, tiers[10]: 100.0})
        return sum(alloc.values()) == 600.0 and t.trial_balance() == 0.0, \
            "surplus flows the policy waterfall; the trial balance is zero"

    def treasury_authority():
        t, ledger = _treasury()
        tiers = t.waterfall
        alloc = t.allocate(350.0, {tiers[0]: 300.0, tiers[1]: 200.0, tiers[10]: 100.0})
        stopped = [r.payload for r in ledger.by_type("event")
                   if r.payload.get("type") == "treasury.waterfall_stopped"]
        return tiers[10] not in alloc and stopped and stopped[0]["stopped_at"] == tiers[1], \
            "an underfunded tier stops the flow; never skip upward to fund downward"

    def treasury_evidence():
        t, ledger = _treasury()
        t.record_debt(kind="attention_drain", severity=0.5, description="manual triage")
        ...
        return bool(events) and set(events[0]["blocks"]) == {
            ...
            "every debt event names exactly what it blocks; accounting that blocks nothing is decoration"

    def treasury_economic():
        from capital.treasury import TreasuryError
        t, ledger = _treasury()
        t.post(debit="operating_cash", credit="revenue", amount_usd=500.0, memo="sale")
        try:
            t.post(debit="a", credit="b", amount_usd=-1.0, memo="hide it")
            return False, "sign game posted"
        except TreasuryError:
            return t.trial_balance() == 0.0, \
                "double entry holds; consumption cannot hide behind output"

    def treasury_regenerative():
        t, ledger = _treasury()
        d = t.record_debt(kind="externalized_risk", severity=0.4, description="risk shifted")
        blocked_before = all(t.blocks(a)[0] for a in
                             ("autonomy_promotion", "budget_expansion", "replication"))
        t.repay_debt(d.debt_id, evidence="risk reconciled; regression test added")
        blocked_after = any(t.blocks(a)[0] for a in
                            ("autonomy_promotion", "budget_expansion", "replication"))
        return blocked_before and not blocked_after, \
            "open debt blocks promotion/expansion/replication; evidence-based repair unblocks"

    reg.register(ModuleClosures("treasury", {
