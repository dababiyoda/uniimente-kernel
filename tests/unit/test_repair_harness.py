...
def run():
    ...
    record = ReplacementExperiment(ledger=ledger, subject=SR002).run()
...
def test_governance_and_continuity_held_while_the_function_was_absent(run):
    ...
    assert record["continuity"]["while_absent"] == SR002.continuity_sha256
    assert record["compatibility_subject"] == compatibility.sr002_subject_record()
...
def test_module_entry_point_emits_a_valid_record_and_exits_zero():
    proc = subprocess.run([sys.executable, "-m", "evolution.repair", "--subject", "sr002"],
