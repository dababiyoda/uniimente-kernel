...
This is a **bounded mechanism experiment**, not a requirement that UNIIMENTE be implemented as digital cells or tissues.

The benchmark contains 120 simulated cells and three simulated tissues:
...
A passing verdict is `MECHANICS_VALIDATED_NOT_PRODUCTION_AUTHORIZED`. It establishes only that the tested mechanism cleared this benchmark. It does not establish consciousness, biological life, sovereignty, production readiness, general morphogenesis, superiority over commodity systems, or achievement of any founder aspiration outside the benchmark.

## Canonical placement

Useful findings from this directory may be extracted into UNIIMENTE when they solve a real CapabilityDeficit better than simpler alternatives.

Examples of transferable mechanisms include:

- local competency under bounded information;
- distributed error correction;
- topology-sensitive resilience;
- target-state regulation;
- graceful degradation;
- local signaling;
- dynamic specialization;
- recovery without exact restoration.

The biological labels are **source-domain vocabulary**. They are not privileged implementation names.

A future agent must not argue "UNIIMENTE needs cells because morphogenesis requires cells." It must instead show that a specific local/distributed mechanism improves a named target effect against a strong conventional baseline.
...
- **Failure modes:** no initial route, no alternate route, incorrect removal fraction, missing tissue, route not blocked, false activation, exact-restoration attempt, recovery timeout, throughput deficit, excessive compute ratio, invalid centralized counterexample, or promotion based on metaphor resemblance rather than target effect.
...
- **Replaceable:** lattice dimensions, transport task, local update algorithm, visualization, simulator runtime, and all biological analogies may change. The evidence, authority, perturbation, baseline, and no-restoration boundaries must remain explicit.

## Promotion rule

No mechanism in this directory becomes canonical merely because it is biologically elegant or passes its own benchmark.

Promotion requires a named external or institutional CapabilityDeficit, a strong simpler baseline, evidence that the mechanism improves the underlying effect, bounded cost/complexity, and preserved authority. Otherwise retain it as research.