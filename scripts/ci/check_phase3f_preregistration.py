#!/usr/bin/env python3
"""Prove the Phase 3F pre-registration from git ancestry, not from self-report.
...
Exit non-zero on any failure. Skips cleanly when the Phase 3F files are absent
...
MANIFEST = "verification/phase3f/EVALUATION_MANIFEST.json"
IMPLEMENTATION = ["substrate/v4.py"]
RESULTS = ["verification/phase3f/PHASE3F_RESULTS.json"]
ADDENDUM = None
...
def main() -> int:
    ...
    if add_commit is not None:
        ...
        for path in ["verification/phase3f/run_phase3f2.py",
                     "verification/phase3f/PHASE3F_RUN2_RESULTS.json"]:
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        return 1
    print("\nOK: Phase 3F thresholds were fixed before the substrate that is "
