...
class BusinessGenome:
    ...
    name: str
    problem: str                         # the recurring problem
    buyer: str                           # who pays (mandate-capable actor)
    offer: str                           # what is sold
    price_usd: float
    distribution: str                    # how buyers arrive (territory, hub, ...)
    conversion: str                      # how attention becomes a customer
    fulfillment: str                     # how delivery happens
    retention: str                       # why they stay
    marginal_cost_usd: float             # cost to serve one customer
    demand_evidence_refs: list[str]      # proof demand exists before software
    required_capabilities: list[tuple[str, str]]   # (name, version) in registry
    required_workflows: list[str]        # ratified Loom pattern hashes
    legal_restrictions: list[str]        # what this business may never do
    regenerative_effect: str             # the gain that does not deplete
    kill_condition: str                  # precommitted; executes without appeal
    falsification_test: str              # cheapest test that could kill it
    ...
    def validate(self) -> list[str]:
        ...
        if self.price_usd <= 0:
            problems.append("price must be positive; free is a distribution "
                            "strategy, not a business")
        if self.marginal_cost_usd < 0:
            problems.append("marginal cost may not be negative")
        if self.price_usd <= self.marginal_cost_usd:
            problems.append(f"price ${self.price_usd} does not cover marginal cost "
                            f"${self.marginal_cost_usd}: a donation with paperwork")
        if not self.demand_evidence_refs:
            problems.append("no demand evidence: do not build software before "
                            "the workflow and demand are proven")
...
class BusinessGenomeCompiler:
    ...
    def __init__(self, *, genome_registry, ratifier, ledger):
        self.genome_registry = genome_registry     # capabilities.GenomeRegistry
        self.ratifier = ratifier                   # loom.Ratifier (any kind)
        self.ledger = ledger
    ...
    def compile(self, genome: BusinessGenome) -> CompiledBusiness:
        ...
        for name, version in genome.required_capabilities:
            if self.genome_registry.get(name, version) is None:
                problems.append(f"required capability {name}@{version} not in registry: "
                                "the institution does not hold that competence")
            else:
        ...
        for pattern_hash in genome.required_workflows:
            ...
            if status != "ratified":
                problems.append(f"required workflow {pattern_hash[:16]}... is "
                                f"{status}, not ratified: a business may not run "
                                "on unratified automation")
            else:
