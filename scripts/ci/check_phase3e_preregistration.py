#!/usr/bin/env python3
"""Prove the Phase 3E pre-registration from git ancestry, not from self-report.
...
Exit non-zero on any failure. Skips cleanly when the Phase 3E files are absent
...
MANIFEST = "verification/phase3e/EVALUATION_MANIFEST.json"
IMPLEMENTATION = ["substrate/v3.py"]
RESULTS = ["verification/phase3e/PHASE3E_RESULTS.json",
           "verification/phase3e/PHASE3E_RUN2_RESULTS.json"]
ADDENDUM = "verification/phase3e/EVALUATION_MANIFEST_2.json"
...
def main() -> int:
    ...
    if add_commit is not None:
        ...
        for path in ["verification/phase3e/run_phase3e2.py",
                     "verification/phase3e/PHASE3E_RUN2_RESULTS.json"]:
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        return 1
    print("\nOK: Phase 3E thresholds were fixed before the substrate that is "
