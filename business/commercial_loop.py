...
Doctrine (loop 7): recurring problem -> buyer -> offer -> payment ->
delivery -> customer outcome -> retention or termination.

Hard rules:
  - the loop advances strictly in order: no delivery before payment,
    no outcome claim before delivery, no retention before a verified
    outcome. Skipping a stage is refused, not warned about;
  - payment and delivery are external/financial effects: each runs
    through the Consequence Gate with its own witness and receipt.
    A payment whose gate run did not reach `recorded` did not happen;
  - a customer outcome is verified by the strongest available verifier;
    self-reported success does not count as verified customer value;
  - a business is not complete because it launches: the loop is CLOSED
    only when a real buyer paid, delivery happened, and the customer's
    outcome was externally verified;
  - the kill condition executes: when the genome's kill condition is
    triggered, the business terminates and the termination is preserved
    as negative evidence (a business that cannot die cannot be trusted).
"""
...
# Loop stages in canonical order. State may only step rightward.
STAGES = ("problem", "buyer", "offer", "payment", "delivery",
          "customer_outcome", "retention_or_termination")

# Verification strengths that count as verified customer value (mission:
# levels 6-7 — same-model critique, intrinsic confidence — never do).
...
@dataclass
class CustomerCase:
    """One buyer moving through the loop for one compiled business."""
    case_id: str
    business_id: str
    buyer: str
    stage: str = "problem"
    ...
    resolution: str | None = None          # retained | terminated
    history: list[dict] = field(default_factory=list)


class CommercialLoop:
    """Runs customer cases for one compiled business through the seven
    stages, with payment and delivery mediated by the Consequence Gate."""
    ...
    def _advance(self, case: CustomerCase, to_stage: str, detail: dict) -> None:
        ...
        if to_stage != expected:
            raise CommercialLoopError(
                f"loop advances in order: at {case.stage!r} the next stage is "
                f"{expected!r}, not {to_stage!r}")
        case.stage = to_stage
    ...
    def _require_alive(self) -> None:
        if self.terminated:
            raise CommercialLoopError(
                f"business terminated ({self.termination_reason}); "
                "a dead business takes no new steps")

    # -- the seven stages --------------------------------------------------
    def open_case(self, buyer: str) -> CustomerCase:
        """Stage 1-2: a recurring problem met a mandate-capable buyer."""
        self._require_alive()
        if not buyer:
    ...
    def present_offer(self, case_id: str) -> CustomerCase:
        """Stage 3: the genome's offer, at the genome's price. No improvising."""
        self._require_alive()
    ...
    def take_payment(self, case_id: str, *, actor: str, executor,
        ...
        """Stage 4: payment is a financial effect through the gate."""
        self._require_alive()
        case = self._cases[case_id]
        if case.stage != "offer":
        ...
        # A costed effect needs budget authorization before evaluation: a
        # single-use grant bound to this exact effect, revalidated at commit.
        ...
        if rec.state != "recorded":
            raise CommercialLoopError(
                f"payment did not reach recorded (state={rec.state}, "
                f"reasons={rec.refusal_reasons}); an unrecorded payment did not happen")
    ...
    def deliver(self, case_id: str, *, actor: str, executor,
                evidence_confidence: float, evidence_refs: list[str],
                approver=None) -> CustomerCase:
        """Stage 5: delivery is an external effect through the gate."""
        self._require_alive()
        case = self._cases[case_id]
        ...
        if rec.state != "recorded":
            raise CommercialLoopError(
                f"delivery did not reach recorded (state={rec.state}, "
                f"reasons={rec.refusal_reasons}); an unrecorded delivery did not happen")
    ...
    def verify_outcome(self, case_id: str, *, verified_by: str,
        ...
        """Stage 6: customer outcome, by an accepted verifier only."""
        self._require_alive()
        case = self._cases[case_id]
        ...
        if verified_by not in ACCEPTED_VERIFICATIONS:
            raise CommercialLoopError(
                f"{verified_by!r} cannot verify customer value; accepted: "
                f"{ACCEPTED_VERIFICATIONS} (self-report and model confidence "
                "generate hypotheses, not verified outcomes)")
    ...
    def resolve(self, case_id: str, *, retained: bool, reason: str) -> CustomerCase:
        """Stage 7: retention or termination. Both are recorded outcomes."""
        self._require_alive()
        case = self._cases[case_id]
    ...
    # -- business-level verdicts -------------------------------------------
    def evaluate(self):
        """Whole-body verdict for loop 7. Internal: cases opened. External:
        at least one case with payment receipt, delivery receipt, AND an
        accepted-verifier customer outcome. Launching is not closure."""
    ...
    def trigger_kill(self, *, evidence: str) -> None:
        """The precommitted kill condition executes. No appeal, preserved."""
        ...
        self.ledger.append("event", {
            ...
            "evidence": evidence,
            "learning": "termination discipline exercised; failure preserved "
